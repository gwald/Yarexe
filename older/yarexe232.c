// Yaroze PS-X EXE builder v2.32 1st July 2019
...
unsigned long endaddress;
    ...
    {
        debugPrint("WARNING: File exceeds memory bounds\n");
...
FILE *handle;
    ...
    strupr2(params[2]);
    debugPrint("Data %-11.11s length %-8.8X address %-8.8X\n", params[2], length,
        address);
...
debugPrint("\n Using Net Yaroze exec = %s\n\n", params[2]);
    ...
    if (!params[2])
    {
        debugPrint("ERROR: Not enough parameters for command DLOAD\n");
        exit(1);
    }
    handle = fopen(params[2], "rb");
    if (!handle)
    ...
    fclose(temphandle);
    strupr2(params[2]);
    debugPrint("Prog %-11.11s length %-8.8X address %-8.8X\n", params[2], length,
        exe_org);
}
...
unsigned char *params[60];
    ...
    strupr2(params[0]);
    if (params[1]) strupr2(params[1]);
    if (!strcmp(params[0], "GO"))
        debugPrint("GO command received - writing to disk...\n");
...
char targetFilename[12] = "combEco.exe";
    ...
    debugPrint("\nLIBPS combine program v2.20\n"
        ...
        "NO WARRANTY.\n\n");
		 ...
		 debugPrint("eco2exe v0.01 Andrew Kieschnick <andrewk@mail.utexas.edu>\n");
		debugPrint("exefixup v0.02 Andrew Kieschnick <andrewk@mail.utexas.edu>\n");

		debugPrint("\n\n        ###### YAREXE ######\n\n");
		 debugPrint("YarEXE Hacked together by gwald @xpcoin\n\n");
		  debugPrint("Syntax: yarEXEC <scriptfile>  [optional: libps.exe file]\n"
    ...
    {
        debugPrint("LIBPS.EXE is corrupt!\n");
        return 1;
    }
...
void exeFixUp_main(char *filenameEXEC)
{
  ...
  debugPrint("id\tPS-X EXE\n");
  debugPrint("text\t0x%.8x\n", headerDATA[0]);
  debugPrint("data\t0x%.8x\n", headerDATA[1]);
  debugPrint("pc0\t0x%.8x\n", headerDATA[2]);
  debugPrint("gp0\t0x%.8x\n", headerDATA[3]);
  debugPrint("t_addr\t0x%.8x\n", headerDATA[4]);
  debugPrint("t_size\t0x%.8x\n", headerDATA[5]);
  debugPrint("d_addr\t0x%.8x\n", headerDATA[6]);
  debugPrint("d_size\t0x%.8x\n", headerDATA[7]);
  debugPrint("b_addr\t0x%.8x\n", headerDATA[8]);
  debugPrint("b_size\t0x%.8x\n", headerDATA[9]);
  debugPrint("s_addr\t0x%.8x\n", headerDATA[10]);
  debugPrint("s_size\t0x%.8x\n\n", headerDATA[11]);
...
debugPrint("sizeEXEC\t %d\n", sizeEXEC);


  padsizeEXE=2048-(sizeEXEC%2048);
debugPrint("padsizeEXE\t %d\n", padsizeEXE);
  
  
  if (padsizeEXE!=2048)
    {
      debugPrint("Creating psx.exe: Fixing EXE size to a multiple of 2048!\n");
	...
	*/
	{
	  ...
	  psxExeCreated++;
		...
		debugPrint("padsizeEXE already 2048 \n");
  
 // yesno='Z';
  
  if (sizeEXEC!=headerDATA[5])
    {
      debugPrint("2 Fixing: EXE header t_size does not match filesize-2048\n");
      debugPrint("EXE header:\t 0x%.8x bytes\n", headerDATA[5]);
      debugPrint("filesize-2048:\t 0x%.8x bytes\n", sizeEXEC);
  /*    while ((yesno!='Y')&&(yesno!='N'))
	{
	...
	psxExeCreated++;
		...
		debugPrint("sizeEXEC already equal to headerDATA[5] \n");
	
	debugPrint("exeFixUp_main finished\n");
 ...
 else
 {
	 ...
	 debugPrint("%s renamed to psx.exe\n", filenameEXEC);	
