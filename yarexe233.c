// Yaroze PS-X EXE builder v2.33  24 Jan 2021
#define YAREXE "YAREXE: Yaroze PS-X EXE builder v2.33 24 Jan 2021"
...
int main(int argc, char *argv[])
{
	...
	printf(YAREXE);
	printf("\nYarEXE Hacked together by gwald aka Mike Garcia\n");

	printf("Copyright is of the respective owners, wherever they are! We thank you!\n\n");

	if( argc < 1)
	{
		printf("Syntax: yarEXEC <scriptfile>  [optional: libps.exe file] [-v: verbose]\n"
	...
	if (!LibPS_fileHnd)
		LibPS_fileHnd = fopen("libps.exe", "rb");
	if (!LibPS_fileHnd)
		LibPS_fileHnd = fopen("Libps.exe", "rb"); //check case sensitive
	if (!LibPS_fileHnd)
		LibPS_fileHnd = fopen("\\Libps.exe", "rb"); //check case sensitive
	if (!LibPS_fileHnd)
		LibPS_fileHnd = fopen("LIBPS.EXE", "rb"); //check case sensitive
	...
	// either lib path or -v

	if( argc > 2)
	{
		if (!LibPS_fileHnd && argv[2]) // still not found, try cmd line
	...
	if( argc > 3)
	{
		if (!LibPS_fileHnd && argv[3]) // still not found, try cmd line
			LibPS_fileHnd = fopen(argv[3], "rb");
